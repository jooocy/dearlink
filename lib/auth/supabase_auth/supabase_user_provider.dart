...
class DearLinkSupabaseUser extends BaseAuthUser {
  DearLinkSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> dearLinkSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = DearLinkSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
